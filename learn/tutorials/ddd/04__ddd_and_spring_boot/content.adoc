...
We should try to keep most of our business logic in entities and value objects. However, there will be situations where a piece of business logic does not naturally fit into a single aggregate. In cases like this you need domain services. Building domain services in a Spring application is very easy - they are just plain old Spring beans. That said, there are a few things to keep in mind.

First, domain services are not secured in anyway. You should be able to invoke them from anywhere inside your application without worrying about the security context. If the domain service really needs knowledge of the current user, it could of course look it up from the `SecurityContextHolder` but a better approach in my opinion is to pass the necessary user information as method parameters. That makes the service more stateless and easier to reuse, for example in a background thread that does not have a security context at all. Invisible contextual information that is transferred under the surface should be kept to a minimum inside the domain model.

Second, domain services do not start transactions, nor do they commit or roll them back. They can participate in existing transactions or  demand that they exist, but that's about it. From a transactional point of view, domain services should always be safe to call without the risk of side effects. It is the caller of the domain service that is responsibe for starting and finishing transactions.

Domain services can publish domain events if they need to, which brings us to our next topic.

== Publishing Domain Events

From Spring's point of view, a domain event is just another application event that can be published using the built-in `ApplicationEventPublisher`. In other words, we do not need to worry about building an event bus or some other infrastructure for publishing domain events: you inject the event publisher into your domain service and publish the event.
...
.PotentiallyProblematicDomainEvent.java
[source,java]
----
public class PotentiallyProblematicDomainEvent implements DomainEvent { // <1>
...
----
<1> The `DomainEvent` interface is just a marker interface that extends the `DomainObject` marker interface.

Whenever you design events like this, you have to be aware of how Spring Data and JPA work under the hood. When you save an existing entity, Spring Data will end up calling `EntityManager.merge`. If the entity is detached, JPA will retrieve the managed entity, copy all the attributes from the detached entity to the managed one, save it and return it. The managed entity will get its optimistic locking version incremented while the detached entity remains untouched. However, since the domain event was registered on the detached entity, the domain event listeners will get a reference to the detached entity. This can lead to optimistic locking errors if a listener try to perform any operations directly on the entity and then save it.
...
.PotentiallyProblematicApplicationService.java
[source,java]
----
...
----
<1> This method accepts an aggregate as a parameter and performs operations on it directly. This means the aggregate is detached and will become stale once saved.
<2> This method accepts the aggregate ID as a parameter and looks up the aggregate before performing operations on it. However, the method is not `@Transactional` which means that both calls to the repository will run inside their own transactions, detaching the aggregate in between.
...
.SaferDomainEvent.java
[source,java]
----
public class SaferDomainEvent implements DomainEvent { 
    private final MyAggregate myAggregate;
...
----
<1> We still store a reference to the aggregate inside the event...
<2> ... but we only expose its ID to the outside world, forcing any listeners to fetch a fresh copy of the aggregate from the repository if they want to do anything with it.

This is yet again an example of the underlying technology silently sneaking into your domain model - your choise of persistence technology can even affect the design of your domain events. Fortunately in this case it is not a big deal, but it is still something that may come back and bite you later if you base your early designs on assumptions that later turn out to be incorrect (I've been there and done that, especially when it comes to JPA). The bottomline is that you need to know your tools well - not only how to use them but also how they work.

== Application Services

We have now looked at all the building blocks for the domain model itself, namely value objects, entities, aggregates, repositories, domain services and domain events. However, a domain model is useless without code that actually interacts with it and this is where application services come in. I covered application services in detail in my previous article about DDD and the hexagonal architecture, but in short, an application service is an API that allows the outside world to interact with your domain model. The application service also controls the transactions and enforces security.

In a Spring application, you implement application services in the same way as you implement domain services: as ordinary Spring beans. I often declare separate interfaces for my application services and keep my implementations hidden using package visibility. This emphasizes the API role that application services have. 

You can handle transactions either programmatically or declaratively. In many real-world applications you often find yourself using both, so don't make any policy decisions from the start that explicitly rules out one in favor of the other. The only thing that is important is that from the caller's point of view, the service call happens inside a transaction that is started when the method call starts and committed when the method call ends (or rolled back if the method throws an exception).

TODO Write something about security as well...

=== Application Service Delegates

TODO What if you need to split up a service into multiple parts?
...
== Next: DDD and Spring Boot From the Trenches

In the next article in this series, we are going to look at some best practices, tips and tricks, and lessons learned from real-world projects. I originally intended to include them in this article, but soon realized they would make this article way too long.
