...
public interface IdentifiableDomainObject<Id> extends DomainObject { // <1>
    @NotNull Optional<Id> getIdOptional(); // <2>

    default @NotNull Id getId() { // <3>
...
<2> I prefer methods that never return null. If you generate the ID when the object is first persisted (as is typical with JPA), you have to deal with the situation that the ID can be null. If you generate the ID when the object is created, you don't have this problem at all.
<3> The only time you would actually use the ID of a domain object is after it has been persisted. Forcing your code to deal with the `Optional` all the time is just a source of irritation and errors (because different developers will treat the `Optional` in different ways).
...
=== Using `AbstractPersistable` and `AbstractAggregateRoot`
