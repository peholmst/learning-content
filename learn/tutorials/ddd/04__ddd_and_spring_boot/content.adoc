...
----
<1> `ValueObject` is an empty marker interface that resides in the `domain.base` package. It is used only for documentational purposes and has no functional meaning. You can leave it out or replace it with a custom annotation if you like to.
